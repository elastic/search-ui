...
import { FacetResponse } from "../facets/FacetResponse";
import { ResponseRequest } from "../Request/Request";
...
function getSummaryFromResponse(
  ...
  const {
    appliedFilters,
    disabledFilters
  }: { appliedFilters: SelectedFilter[]; disabledFilters: DisabledFilter[] } =
    filters.reduce(
      (sum, filter) => {
...
export class ElasticSearchResponseTransformer
  implements SearchkitResponseTransformer
{
  transformResponse(
    ...
    const facets = responseRequest.facets
      ...
      : null;
    const summary = getSummaryFromResponse(
      responseBody,
      facetsConfig,
      queryManager,
      config
