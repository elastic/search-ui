...
export class SearchkitRequest {
  ...
  constructor(
    ...
    this.transporter = !transporter
      ? new FetchClientTransporter(config)
      : transporter;
  ...
  handleDefaults(): void {
    const defaultSortOptionId =
      ...
      this.config.sortOptions.find((option) => option.defaultOption);
    const defaultSortOption =
      defaultSortOptionId &&
      getSortOption(defaultSortOptionId.id, this.config.sortOptions);
  ...
  async executeSuggestions(query: string): Promise<Array<any>> {
    const suggestions = await Promise.all(
      this.config.suggestions.map((suggestor) =>
        suggestor.getSuggestions(query, this.transporter)
      )
    );
    return suggestions;
  }
  ...
  async execute(
    ...
    if (responseRequest.facets) {
      if (Array.isArray(responseRequest.facets)) {
        const filteredFacetIdentifiers = responseRequest.facets.map(
          (facet) => facet.identifier
        );
        filteredFacets =
          facets &&
          facets.filter((facet) =>
            filteredFacetIdentifiers.includes(facet.getIdentifier())
          );
