  ...
  const aggBuckets = facetsConfig.reduce(
    (buckets, facet) => {
      if (facet.excludeOwnFilters && queryManager.hasFilters()) {
        ...
        const combinedBucket = buckets.find(
          ({ name }) => name === "facet_bucket_all"
        );
    ...
    const subAggs = bucket.aggs.reduce(
      (subAggs, subAgg: BaseFacet) => ({
        ...
        ...subAgg.getAggregation(
          overrides[subAgg.getIdentifier()],
          queryManager
        )
      }),
      {}
...
export const getFacetsFromResponse = (
  ...
  const collapsedFacetAggsMap = facetBucketKeys.reduce(
    (facetAggsResponse, bucketKey) => {
