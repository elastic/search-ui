...
A "Headless Search UI". It handles all of the state management, API calls, and url management concerns of a search experience, and simply exposes that `state` object and a group of `actions` to act on that state. It is built with plain old javascript, and serves as the base for our [React](packages/react-search-components) and [JQuery](packages/jquery-search-components) Framework Implementations.
