...
1. Copy the code snippet and paste it into your application. Put it before the `ReactDOM.render()` call, so APM RUM would already be initialized when React starts rendering the application.
...
If you're using Search UI in React application, it's recommended to also use the React integraton of APM RUM. Follow this guide for setting it up: https://www.elastic.co/guide/en/apm/agent/rum-js/current/react-integration.html
