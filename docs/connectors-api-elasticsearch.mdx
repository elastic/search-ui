...
const connector = new ElasticSearchAPIConnector(
  ...
  (requestBody) => {
    console.log("postProcess Query Call", requestBody); // logging out the esQuery before sending to Elasticsearch
    if (!requestBody.query) return requestBody;

    const bestMatchQuery = requestBody.query.bool.should[0].multi_match;
    requestBody.query = {
      multi_match: {
        query: bestMatchQuery.query,
        fields: bestMatchQuery.fields
      }
    };
