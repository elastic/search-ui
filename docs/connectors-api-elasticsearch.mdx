...
const connector = new ElasticSearchAPIConnector(
  ...
  (esQuery) => {
    console.log("postProcess Query Call", esQuery); // logging out the esQuery before sending to Elasticsearch
    if (!esQuery.query) return esQuery;

    const bestMatchQuery = esQuery.query.bool.should[0].multi_match;
    esQuery.query = {
      multi_match: {
