...
Next we setup Search UI `Searchbox` and `configuration` to display suggestions from the `popular_queries` index.
...
<DocTabs>
  ...
  <DocTab name="Searchbox Component">
      ...
      <SearchBox autocompleteSuggestions={{
          popularQueries: {
            queryType: "results",
            displayField: "name" // specify which field used to display the suggestion
          }}} autocompleteView={AutocompleteView} />;
      ```
  </DocTab>
  <DocTab name="Autocomplete View Component">
