...
title: "Autocomplete"
description: Providing suggestions to customers as they type in a search query.
date: 2022-07-16
tags: ["autocomplete"]
---

When you start typing in a search box on ecommerce sites like Amazon or Best Buy, you might have seen a drop-down showing you suggestions to try next. This is called autocomplete.
...
In this article we will discuss how to implement an autocomplete search box that provides multiple options on your ecommerce site.

Search UI can be used to provide different types of suggestions from multiple sources. Depending on your specific use-case, you can display query suggestions, products from the product catalog, or a list of popular queries to your customer at the same time.
...
Typically ecommerce sites have a search bar that is present within the site's header and accessible everywhere.

For the purposes of this tutorial, we will assume that the search bar is located in the header of the site.
...
With this feature, products will be presented as suggestions to the customer. When the customer clicks on the product suggestion. they will be navigated straight to the product's detail page. This helps your customer quickly find the product they are looking for and be quickly redirected to the product detail page.

![Search suggestions with Search UI](images/ecommerce/autocomplete/query-results-es-shop.png)

To this, we start by specify the autocompleteQuery `results` configuration
...
Sometimes you want to display suggestions from a different index than the one you are searching. For example, you might want to display suggestions from a `popular_queries` or a `designers` index. Search UI supports this within the `autocompleteSuggestions` configuration.
...
Now when you type `Iphone 4s` in the searchbox, the autocomplete view will display the popular queries.
...
With a combination of the suggestion configurations above, you can display suggestions from multiple sources at the same time.

![Suggestions from multiple sources](images/ecommerce/autocomplete/query-suggestions-multiple-sources.png)

To do this, extend the `autocompleteQuery` configuration to specify multiple sources. For the above screenshot, we customised the `autocompleteView` css to display the popular queries and the results from the `autocompleteSuggestions` configuration side by side and hide the section titles.
