...
Carousels can be used to show groups of products into a row. Typivcally used to show products within a certain category or tag, like "most popular", "best rated" and "on sale".

In this example, we show a carousel of products that are within the "TVs" category using the `Results` component and overriding the `view`.

You can adjust the number of results brought back via the `resultsPerPage` configuration.
...
<DocRelatedArticles
  ...
  items={[
    {
      pageId: "api-react-components-results"
    },
    {
