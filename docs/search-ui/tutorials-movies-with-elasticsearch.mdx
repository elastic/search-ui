...
First we need to setup elasticsearch. The easiest way to do this is to create an elasticsearch instance via [cloud](https://cloud.elastic.co/registration).

We are going to issue commands via the dev tools that's accessible within kibana. You can alternatively use a REST client like Postman to achieve this.

Next we need to create an index for our data. We can do this simply via the following request:
...
so the mapping file will be as follows and we use the kibana dev tools to update the mapping file for our index.
...
You can find your elasticsearch host in deployment if your hosted your elasticsearch on cloud.
...
- update the Searchbox to switch off autocomplete
- remove sorting options
- add facet component for each facet field
- update results component to display all the fields
...
Next you can add more data into the index, <DocLink id="guides-customizing-styles-and-html" section="results-component" text="update the results view to display more fields" />, and test the app.
