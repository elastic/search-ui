...
slug: /tutorials/workplace-search
...
We'll use React components in this tutorial, but you can use Search UI with any UI framework and even vanilla JavaScript. Read our <DocLink id="guides-philosphy" /> to learn more about using Search UI outside of React.

First, let's install Search UI packages:

```shell
npm install @elastic/search-ui @elastic/react-search-ui @elastic/react-search-ui-views @elastic/search-ui-workplace-search-connector
```
