...
| `spellingSuggestion` | Object | Spelling Suggestion returned by the Site Search API. Populated if `spelling` configured in [queryConfig](#query-config). Object has a `text` field with the suggested text as a string, and a numeric `score` field. Example: `{"text": "support","score": 0.036801625}` |
...
| `spelling`                    | Object  | optional  |         | **Only supported by Site Search** <br/> [Site Search API reference](https://swiftype.com/documentation/site-search/searching/spelling) <br/> Specify the type of spelling behaviour with a `type` field that should have a value of "strict", "always" or "retry" <br/> Example: ```spelling: { type: "strict" }``` |
